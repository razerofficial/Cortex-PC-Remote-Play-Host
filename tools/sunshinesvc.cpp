 ...
 * @brief Handles launching RazerRemotePlayHost.exe into user sessions as SYSTEM
 */
...
#define SERVICE_NAME    "RazerRemotePlayHostService"
#define SERVICE_NAME_W  L"RazerRemotePlayHostService"
...
CreateJobObjectForChildProcess() {
  ...
  // Kill RazerRemotePlayHost.exe when the final job object handle is closed (which will happen if we terminate unexpectedly).
  // This ensures we don't leave an orphaned RazerRemotePlayHost.exe running with an inherited handle to our log file.
  job_limit_info.BasicLimitInformation.LimitFlags |= JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE;

  // Allow RazerRemotePlayHost.exe to use CREATE_BREAKAWAY_FROM_JOB when spawning processes to ensure they can to live beyond
  // the lifetime of RazerRemotePlayHostService.exe. This avoids unexpected user data loss if we crash or are killed.
...
OpenLogFileHandle() {
  ...
  // Create RemotePlayHost.log in the Temp folder (usually %SYSTEMROOT%\Temp)
  GetTempPathW(_countof(log_file_name), log_file_name);
  GetTempPathW(_countof(log_file_name), log_file_name);
  wcscat_s(log_file_name, L"sunshine_");
  wcscat_s(log_file_name, SERVICE_NAME_W);
  wcscat_s(log_file_name, L".log");
  ...
  // Overwrite the old RemotePlayHost.log
...
ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) {
  ...
  // Only allow RazerRemotePlayHost.exe to inherit the log file handle, not all inheritable handles
  ...
  // Loop every 3 seconds until the stop event is set or RazerRemotePlayHost.exe is running
  //while (WaitForSingleObject(stop_event, 3000) != WAIT_OBJECT_0) {
    ...
    // Start RazerRemotePlayHost.exe inside our job object
    UpdateProcThreadAttribute(startup_info.lpAttributeList,
      0,
    ...
    bool still_running;
    do {
      // Wait for the stop event to be set, RazerRemotePlayHost.exe to terminate, or the console session to change
      ...
      switch (WaitForMultipleObjects(_countof(wait_objects), wait_objects, FALSE, INFINITE)) {
        case WAIT_OBJECT_0 + 2:
          ...
          // Fall-through to terminate RazerRemotePlayHost.exe and start it again.
        case WAIT_OBJECT_0:
          // The service is shutting down, so try to gracefully terminate RazerRemotePlayHost.exe.
...
main(int argc, char *argv[]) {
  ...
  // Check if this is a reinvocation of ourselves to send Ctrl-C to RazerRemotePlayHost.exe
  ...
  // We want to use the directory where RazerRemotePlayHost.exe is located instead of system32.
