...
if exist "%OLD_DIR%\RemotePlayHost.conf" (
    if not exist "%NEW_DIR%\RemotePlayHost.conf" (
        move "%OLD_DIR%\RemotePlayHost.conf" "%NEW_DIR%\RemotePlayHost.conf"
        icacls "%NEW_DIR%\RemotePlayHost.conf" /reset
    )
)
if exist "%OLD_DIR%\RemotePlayHost_state.json" (
    if not exist "%NEW_DIR%\RemotePlayHost_state.json" (
        move "%OLD_DIR%\RemotePlayHost_state.json" "%NEW_DIR%\RemotePlayHost_state.json"
        icacls "%NEW_DIR%\RemotePlayHost_state.json" /reset
    )
)
