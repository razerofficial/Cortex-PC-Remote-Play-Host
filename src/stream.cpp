      ...
      switch (event.type) {
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
          ...
          if (session->state == session::state_e::RUNNING) {
            #if defined RAZER_MOD && RAZER_MOD == 1
            if(proc::proc.get_last_run_app_name() == "Desktop"){
              proc::proc.terminate(); // dekstop do not needs pause session
            }
            else{
              if(rz_state::GetCountdownTimer() == "0"){
                proc::proc.terminate(); // stop now.
              }
              else
                rz_state::StartGameCloseCountdown();
            }
            #endif
    ...
    join(session_t &session) {
      ...
      if (--running_sessions == 0) {
        ...
        if (proc::proc.running()) {
          ...
          #endif
          #if defined RAZER_MOD && RAZER_MOD == 1
          rz_state::updateCurrentState("StreamPause", proc::proc.get_last_run_app_name(), current_device_nickname);
          rz_state::updateEvent(Command_Protocol_VirtualDisplay_Plugout, "", "");
          #endif
        }
#endif
...
#endif
#if defined RAZER_MOD && RAZER_MOD == 1
        rz_state::updateCurrentState("StreamPlay", proc::proc.get_last_run_app_name(), current_device_nickname);        
#endif
      }
