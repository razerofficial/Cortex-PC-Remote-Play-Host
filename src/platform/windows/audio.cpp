...
#define STEAM_STREAMING_SPEAKERS_NAMEW  L"Steam Streaming Speakers"sv
#define STEAM_STREAMING_SPEAKERS_NAMEA  "Steam Streaming Speakers"s

#define RAZER_SPEAKERS_NAMEW  L"Razer Remote Play Speaker"sv
#define RAZER_SPEAKERS_NAMEA  "Razer Remote Play Speaker"s
...
namespace platf::audio {
  ...
  public:
    std::optional<sink_t>
    sink_info() override {
      auto virtual_adapter_name = STEAM_STREAMING_SPEAKERS_NAMEW;//L"Steam Streaming Speakers"sv;
      auto select_adapter_name = L"";
      sink_t sink;
      ...
      std::wstring virtual_device_id;
      if(config::audio.virtual_sink != "Auto"){
        virtual_device_id = find_device_id_by_name(config::audio.virtual_sink).value_or(from_utf8(config::audio.virtual_sink));
        BOOST_LOG(info) << "virtual device id = " << virtual_device_id; 
      }
      else
      {
        BOOST_LOG(info) << "virtual device id empty";
        virtual_device_id = L"";
      }

      //
      auto found_by_id = false;
      ...
      for (auto x = 0; x < count; ++x) {
       ...
       if (virtual_device_id == device_id) {
          found_by_id = true;
          virtual_device_found = true;
          select_adapter_name = adapter_name;
          BOOST_LOG(info) << "device ID:" << virtual_device_id << " found.";
          break;
        }
      }

      if(found_by_id == false){ //pure id found fails, we try find by adapter name
        // 1. find steam first
        BOOST_LOG(info) << "device ID:" << virtual_device_id << " not found.";
        BOOST_LOG(warning) << " - try to find " << STEAM_STREAMING_SPEAKERS_NAMEA;
        virtual_device_id = L""; // clean it
        for (auto x = 0; x < count; ++x) {
          audio::device_t device;
          collection->Item(x, &device);
        ...
        if (!virtual_device_found){
          virtual_adapter_name = RAZER_SPEAKERS_NAMEW;
          // not found steam audio
          BOOST_LOG(warning) << " - Not found " << STEAM_STREAMING_SPEAKERS_NAMEA << " try to find " << virtual_adapter_name;
          for (auto x = 0; x < count; ++x) {
            audio::device_t device;
            collection->Item(x, &device);
            ...
            if (virtual_device_id.empty() &&  std::wstring_view(adapter_name).find(virtual_adapter_name) != std::string::npos) {
              virtual_device_id = std::move(device_id);
              virtual_device_found = true;
              select_adapter_name = adapter_name;
              break;
      ...
      if (virtual_device_found) {
        BOOST_LOG(info) << "final select output device is " << select_adapter_name;
        sink.foundVirtualSinkFriendlyName = select_adapter_name;        
        ...
        BOOST_LOG(warning) << "Unable to find the any specified virtual sink: "sv;
      ...
      for (int x = 0; x < (int) ERole_enum_count; ++x) {
        ...
        if (status) {
          ...
          if (status == HRESULT_FROM_WIN32(ERROR_NOT_FOUND) || status == E_INVALIDARG) {
            BOOST_LOG(info) << "Audio sink not found: "sv << sink;
          }
          else {
            BOOST_LOG(info) << "Couldn't set ["sv << sink << "] to role ["sv << x << "]: 0x"sv << util::hex(status).to_string_view();
          }
    ...
    find_device_id_by_name(const std::string &name) {
      ...
      for (auto x = 0; x < count; ++x) {
        ...
        if (std::wstring(adapter_name).find(wstring_name) != std::string::npos ||
            //std::wcscmp(wstring_name.c_str(), adapter_name) == 0 ||
...
#if RAZER_MOD == 1
    /**
     * @brief Resets the default audio device from Remote Play
     */
    void
    reset_default_device_from_razer() {
      auto razer_device_id = find_device_id_by_name(RAZER_SPEAKERS_NAMEA);
      if (!razer_device_id) {
        return;
      }

      {
        ...
        // If Remote Play Speaker are already not default, we're done.
        if (*razer_device_id != current_default_id.get()) {
          return;
        }
      }

      // Disable the Remote Play Speaker temporarily to allow the OS to pick a new default.
      auto hr = policy->SetEndpointVisibility(razer_device_id->c_str(), FALSE);
      if (FAILED(hr)) {
        BOOST_LOG(warning) << "Failed to disable Razer audio device: "sv << util::hex(hr).to_string_view();
        return;
      }
      ...
      // Enable the Remote Play Speaker again
      hr = policy->SetEndpointVisibility(razer_device_id->c_str(), TRUE);
      if (FAILED(hr)) {
        BOOST_LOG(warning) << "Failed to enable Razer audio device: "sv << util::hex(hr).to_string_view();
        return;
      }

      // If there's now no audio device, the Remote Play Speaker were the only device available.
      ...
      BOOST_LOG(info) << "Successfully reset default audio device to "sv << RAZER_SPEAKERS_NAMEA;
    }

    /**
     * @brief Installs the Razer speakers driver
     * @return `true` if installation was successful.
     */
    // bool
    // install_razer_audio_drivers() {
    ...
    reset_default_device() {
      auto steam_device_id = find_device_id_by_name(STEAM_STREAMING_SPEAKERS_NAMEA/*"Steam Streaming Speakers"s*/);
      if (!steam_device_id) {
        return;
      }
...
namespace platf {
  ...
  audio_control() {
...
#if RAZER_MOD == 0 // not try to help install
    ...
    if (config::audio.install_steam_drivers && !control->find_device_id_by_name(STEAM_STREAMING_SPEAKERS_NAMEA/*"Steam Streaming Speakers"s*/)) {
      // This is best effort. Don't fail if it doesn't work.
      control->install_steam_audio_drivers();
    }
#else
    BOOST_LOG(info) << "try to find device id by name " << STEAM_STREAMING_SPEAKERS_NAMEA;
    if (!control->find_device_id_by_name(STEAM_STREAMING_SPEAKERS_NAMEA)) {
      BOOST_LOG(info) << "try to find device id by name " << RAZER_SPEAKERS_NAMEA;
      if(!control->find_device_id_by_name(RAZER_SPEAKERS_NAMEA)){
        BOOST_LOG(info) << "find device id by name " << RAZER_SPEAKERS_NAMEA;
        // unselect it
        control->reset_default_device_from_razer();
        BOOST_LOG(info) << "unselected it";
      }
    }
#endif
...
class AudioEndpointManager {
    public:
        AudioEndpointManager() : savedDeviceId(nullptr) {}

        ~AudioEndpointManager() {
            if (savedDeviceId) {
                CoTaskMemFree(savedDeviceId);
            }
        }

        bool SaveDefaultAudioEndpoint() {
            HRESULT hr;

            auto deviceEnumerator = CreateCOMObject<IMMDeviceEnumerator>(__uuidof(MMDeviceEnumerator));
            if (!deviceEnumerator) {
                BOOST_LOG(error) << "Failed to create IMMDeviceEnumerator." << std::endl;
                return false;
            }

            std::shared_ptr<IMMDevice> defaultDevice;
            hr = deviceEnumerator->GetDefaultAudioEndpoint(eRender, eConsole, WrapCOMPointer(defaultDevice));
            if (FAILED(hr)) {
                BOOST_LOG(error) << "Failed to get default audio endpoint. Error: " << hr << std::endl;
                return false;
            }

            hr = defaultDevice->GetId(&savedDeviceId);
            if (FAILED(hr)) {
                BOOST_LOG(error) << "Failed to get device ID. Error: " << hr << std::endl;
                return false;
            }

            BOOST_LOG(info) << L"Saved default audio endpoint: " << savedDeviceId << std::endl;
            return true;
        }

        bool RestoreDefaultAudioEndpoint() {
            if (!savedDeviceId) {
                BOOST_LOG(error) << "No saved audio endpoint to restore." << std::endl;
                return false;
            }
            ...
            if (FAILED(status)) {
                BOOST_LOG(error) << "Couldn't create audio policy config. Error: 0x"
                    << std::hex << status << std::endl;
                return false;
            }

            HRESULT hr = policy->SetDefaultEndpoint(savedDeviceId, eConsole);
            if (FAILED(hr)) {
                BOOST_LOG(error) << "Failed to set default audio endpoint. Error: 0x"
                    << std::hex << hr << std::endl;
                policy->Release();
                return false;
            }

            BOOST_LOG(info) << L"Restored default audio endpoint: " << savedDeviceId << std::endl;
    ...
    private:
        LPWSTR savedDeviceId;

        template <typename T>
        std::shared_ptr<T> CreateCOMObject(REFCLSID clsid) {
            T* rawPtr = nullptr;
            HRESULT hr = CoCreateInstance(clsid, nullptr, CLSCTX_ALL, __uuidof(T), (void**)&rawPtr);
            if (FAILED(hr)) {
                return nullptr;
            }
        ...
        template <typename T>
        T** WrapCOMPointer(std::shared_ptr<T>& comPtr) {
            return reinterpret_cast<T**>(&comPtr);
        }  
    };

  AudioEndpointManager _manager;
  bool _bNeedRestoreAudio = false;
  int SaveCurrentAudioDevice(){
    BOOST_LOG(info) << " Need to save current audio device before streaming";
    _bNeedRestoreAudio = _manager.SaveDefaultAudioEndpoint();
    return 0;
  }

  int RestoreAudioDevice(){
    BOOST_LOG(info) << " restore audio device after streaming?";
    if(_bNeedRestoreAudio){
      _manager.RestoreDefaultAudioEndpoint();
      _bNeedRestoreAudio = false; //reset
    }
    return 0;
  }
}  // namespace platf
