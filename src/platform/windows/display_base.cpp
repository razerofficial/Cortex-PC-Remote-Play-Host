...
namespace platf::dxgi {
  ...
  validate_and_test_gpu_preference(const std::string &display_name, bool verify_frame_capture) {
    ...
    // GPU driver overrides than RazerRemotePlayHost.exe, we want to avoid a scenario where
...
// On hybrid graphics systems, Windows will change the order of GPUs reported by
  ...
  display_base_t::init(const ::video::config_t &config, const std::string &display_name) {
    ...
    {
      BOOST_LOG(error) << "display parram error, not capture and stream";
      return -1;
    }
    #endif
...
#if RAZER_MOD == 1
  bool CheckPrimaryScreenHDR(bool& isHdrSupported, bool& isHdrEnabled) {
    isHdrSupported = false; // Default to no HDR support
    isHdrEnabled = false;   // Default to HDR not enabled

    // Create a DXGI factory
    IDXGIFactory6* factory = nullptr;
    if (FAILED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
        BOOST_LOG(error) << "Failed to create DXGI factory." << std::endl;
        return false;
    }

    // Enumerate the first adapter (primary GPU)
    IDXGIAdapter1* adapter = nullptr;
    if (FAILED(factory->EnumAdapters1(0, &adapter))) {
        BOOST_LOG(error) << "Failed to enumerate adapters." << std::endl;
        factory->Release();
        return false;
    }

    // Enumerate the first output (primary screen)
    IDXGIOutput* output = nullptr;
    if (FAILED(adapter->EnumOutputs(0, &output))) {
        BOOST_LOG(error) << "Failed to enumerate outputs." << std::endl;
        adapter->Release();
        factory->Release();
        return false;
    }

    // Query for the DXGIOutput6 interface to check HDR
    IDXGIOutput6* output6 = nullptr;
    if (FAILED(output->QueryInterface(IID_PPV_ARGS(&output6)))) {
        BOOST_LOG(error) << "The output does not support DXGIOutput6." << std::endl;
        output->Release();
        adapter->Release();
        factory->Release();
        return false;
    ...
    // Get the detailed description of the display
    DXGI_OUTPUT_DESC1 desc1;
    if (FAILED(output6->GetDesc1(&desc1))) {
        BOOST_LOG(error) << "Failed to get output description." << std::endl;
        output6->Release();
        output->Release();
        adapter->Release();
        factory->Release();
    ...
    isHdrSupported = (desc1.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020);

    // Check if HDR is currently enabled
    isHdrEnabled = (desc1.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020);
