...
namespace {
  ...
  const auto sunshine_application_profile_name = L"RazerRemotePlayHostStream";
  const auto sunshine_application_path = L"RazerRemotePlayHost.exe";
...
namespace nvprefs {
  ...
  driver_settings_t::check_and_modify_application_profile(bool &modified) {
    ...
    if (!get_nvprefs_options().sunshine_high_power_mode) {
      if (status == NVAPI_OK &&
          setting.settingLocation == NVDRS_CURRENT_PROFILE_LOCATION) {
        // User requested to not use high power mode for RazerRemotePlayHost.exe,
