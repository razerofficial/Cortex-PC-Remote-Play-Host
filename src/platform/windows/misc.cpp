...
bp::child
  create_boost_child_from_results(bool process_launched, const std::string &cmd, std::error_code &ec, DWORD dwPid) {
    if (ec) {
      // If there was an error, return an empty bp::child object
      return bp::child();
    }
    ...
    if (process_launched) {
      // If the launch was successful, create a new bp::child object representing the new process
      auto child = bp::child((bp::pid_t)dwPid);
      BOOST_LOG(info) << cmd << " running with PID "sv << child.id();
      return child;
    }
    else {
  ...
  create_startup_info(FILE *file, HANDLE *job, std::error_code &ec) {
    ...
    if (file) {
      ...
      // our inheritable handles, such as our own log file handle created by RazerRemotePlayHostService).
      //
...
#if RAZER_MOD == 1
  bp::child
  launchMicrosoftStoreApp(const std::string& packageID, std::error_code& ec){
    std::string appFullName = packageID;
    DWORD pid = 0;
    PACKAGE_INFO_REFERENCE packageInfoReference;
    HRESULT hr = OpenPackageInfoByFullName(from_utf8(appFullName).c_str(), 0, &packageInfoReference);    
    if (hr == S_OK)
    {
        UINT32 appIdsCount = 0;
        UINT32 length = 0;
        hr = GetPackageApplicationIds(packageInfoReference, &length, nullptr, &appIdsCount);
        if (appIdsCount > 0)
        {
            BYTE* buffer = (BYTE*)malloc(length);
            hr = GetPackageApplicationIds(packageInfoReference, &length, buffer, &appIdsCount);
            if (hr == S_OK)
            {
                PCWSTR* applicationUserModelIds = (PCWSTR*)buffer;
                if (applicationUserModelIds && applicationUserModelIds[0])
                {
                    std::wstring appId = applicationUserModelIds[0];
                    IApplicationActivationManager* activationManager;
                    hr = ::CoCreateInstance(
                        CLSID_ApplicationActivationManager, nullptr, CLSCTX_ALL,
                    ...
                    if (hr == S_OK)
                    {
                        ...
                        hr = activationManager->ActivateApplication(
                            appId.c_str(), L"", AO_NOERRORUI, &pid);
                    }
                    if (activationManager != nullptr) {
              ...
              BOOST_LOG(error) << "Run Microsoft store GetPackageApplicationIds failed: " << hr;
            if(buffer)
              free(buffer);
        }
        else
          BOOST_LOG(error) << "Run Microsoft store GetPackageApplicationIds failed: " << hr << " , app count " << appIdsCount;

        ClosePackageInfo(packageInfoReference);

        return create_boost_child_from_results(hr==S_OK, packageID, ec, pid);
    }
    else
    {
      BOOST_LOG(error) << "Run Microsoft store OpenPackageInfoByFullName failed: " << hr;
      ec = std::make_error_code(std::errc::bad_file_descriptor);
      return bp::child();
    }
  }

  int GetAppRunningPID(const std::string &fullPath) {
    ...
    if (Process32First(snapshot, &processEntry)) {
        do {
            if (fileName == processEntry.szExeFile) {
  ...
  bp::child
  run_MS_store_command(const std::string &packageID, std::error_code &ec, bp::group *group){

    if(packageID.find("MicrosoftSolitaireCollection") != std::string::npos){ 
      int pid = GetAppRunningPID("Solitaire.exe");
      if(pid > 0){
        BOOST_LOG(info) << "MicrosoftSolitaireCollection running as " << pid;
        return create_boost_child_from_results(true, packageID, ec, pid);
      }
    }
    ...
    if (is_running_as_system()) {
      ...
      if (WTSQueryUserToken(WTSGetActiveConsoleSessionId(), &userToken)) {
          if (ImpersonateLoggedOnUser(userToken)) {
              auto ret = launchMicrosoftStoreApp(packageID, ec);
              RevertToSelf();
              return ret;
          } else {
              BOOST_LOG(error) << "ImpersonateLoggedOnUser failed: " << GetLastError();
          }
          CloseHandle(userToken);
      } else {
          BOOST_LOG(error) << "WTSQueryUserToken failed: " << GetLastError();
      }
    }
    else{
      return launchMicrosoftStoreApp(packageID, ec);
    }
  ...
  streaming_will_start() {
    BOOST_LOG(info) << "streaming will start";
  ...
  void
  streaming_will_stop() {
    BOOST_LOG(info) << "streaming will stop";
  ...
  void force_shutdown(int time_in_second)
  {
