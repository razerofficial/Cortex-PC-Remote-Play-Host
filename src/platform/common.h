...
#undef _CONVERT
  ...
  struct sink_t {
    ...
    std::string host;
#if RAZER_MOD == 1
    std::wstring foundVirtualSinkFriendlyName;
#endif
...
#if RAZER_MOD == 1
  boost::process::child
  run_MS_store_command(const std::string &packageID, std::error_code &ec, boost::process::group *group);

  // Function to check if the primary screen supports HDR and if HDR is enabled
  ...
  shutdown();
  void
  force_shutdown(int time_in_second);
...
#define SERVICE_NAME "remoteplay"
#define SERVICE_TYPE "_rzstream._tcp"
