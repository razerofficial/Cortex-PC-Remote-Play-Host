...
#undef _CONVERT_
  ...
  nvhttp_t nvhttp {
    ...
    CERTIFICATE_FILE,
#if RAZER_MOD == 1
    rz_state::GetUTF8HostName(),
#else
    boost::asio::ip::host_name(),  // sunshine_name,
#endif
    "RemotePlayHost_state.json"s,  // file_state
    {},  // external_ip
    {
      "352x240"s,
      "480x360"s,
  ...
  sunshine_t sunshine {
    ...
    {},  // Password
    {},  // Password Salt
    platf::appdata().string() + "/RemotePlayHost.conf",  // config file
#if RAZER_MOD
    platf::appdata().string() + "/RemotePlayHost_default.conf",
#endif
    {},  // cmd args
    51337,  // Base port number
    "ipv4",  // Address family
    platf::appdata().string() + "../../../../Log/RemotePlayHost.log",  // log file
    {},  // prep commands
    #if RAZER_MOD == 1
    "razertoken",  // razer pair token
    #endif
  };

#if RAZER_MOD == 1
razerSetting_t razerSettings {
  "Manual" //razer id pair
};
#endif
  ...
  apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    #if RAZER_MOD == 1
    string_f(vars, "RazerIdPairing", razerSettings.IDPairing);
    #endif
  }
  ...
  reset_config(){
      ...
      if (fs::exists(sunshine.config_file)) {
        fs::remove(sunshine.config_file);
  ...
  parse(int argc, char *argv[]) {
    ...
    try {
      ...
      if (!fs::exists(sunshine.config_file)) {
        ...
        if (!fs::exists(sunshine.config_default_file)){
          std::filesystem::copy(sunshine.config_default_file, sunshine.config_file, fs::copy_options::overwrite_existing);
        }
        else{
          std::ofstream { sunshine.config_file };
        }
