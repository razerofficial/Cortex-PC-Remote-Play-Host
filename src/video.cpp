...
namespace video {
  ...
  make_avcodec_encode_session(platf::display_t *disp, const encoder_t &encoder, const config_t &config, int width, int height, std::unique_ptr<platf::avcodec_encode_device_t> encode_device) {
    ...
    BOOST_LOG(info) << "make_avcodec_encode_session encoder name = " << video_format.name;
  ...
  probe_encoders() {
...
#if RAZER_MOD == 1
    //virtual display driver needs to know the encoder to adjust the adpater.
    std::string EncoderFlag;
    if(encoder.name == "nvenc")
      EncoderFlag = "NVIDIA";
    else if(encoder.name == "quicksync")
      EncoderFlag = "Intel";
    else if(encoder.name == "amdvce")
      EncoderFlag = "AMD";
    else
      EncoderFlag = "Intel"; //software
    BOOST_LOG(info) << "Set Virtual display encoder adpater: "sv << EncoderFlag;
    ...
    if (programData) {
        ...
        programDataPath /= "AdapterConfig.txt";
        BOOST_LOG(info) << "ProgramData Path: " << programDataPath;
        file_handler::write_file(programDataPath.string().c_str(), EncoderFlag);
    }
    else {
       BOOST_LOG(info) << "Unable to get ProgramData path.";
    }
#endif
