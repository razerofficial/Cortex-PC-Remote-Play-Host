...
namespace audio {
  ...
  capture(safe::mail_t mail, config_t config, void *channel_data) {
    ...
    if (!ref->sink_flag->exchange(true, std::memory_order_acquire)) {      
      ...
      BOOST_LOG(info) << "ref->sink.host " << ref->sink.host << ", to set sink " << *sink;
  ...
  stop_audio_control(audio_ctx_t &ctx) {
    ...
    if (!ctx.restore_sink) {
      BOOST_LOG(info) << "Not need to restore";
      return;
    }

    // Change back to the host sink, unless there was none
    BOOST_LOG(info) << "Config audio sink host " << config::audio.sink;
    BOOST_LOG(info) << "Change back to sink host " << ctx.sink.host;
    const std::string &sink = ctx.sink.host.empty() ? config::audio.sink : ctx.sink.host;    
    if (!sink.empty()) {
      // Best effort, it's allowed to fail
      //std::thread delaySetback([&]() {
      int count = 30;
      while(count){
        std::this_thread::sleep_for(1s);
        if(ctx.control->set_sink(sink) == 0)
          break;
        count--;   
      }
      if(count <= 0)
        BOOST_LOG(info) << " Sink changed timeout" ;
      else
        BOOST_LOG(info) << " Sink changed success" ;
      //});
      //delaySetback.detach();
    }
  }
