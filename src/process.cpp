...
#if defined RAZER_MOD && RAZER_MOD == 1
extern std::string current_device_nickname;
#endif
...
namespace proc {
  ...
  bool isInList(const std::string& target, const std::initializer_list<std::string>& list) {
    return std::any_of(list.begin(), list.end(), [&target](const std::string& s) {
        return s == target;
    });
  }

  bool isInListContain(const std::string& target, const std::initializer_list<std::string>& list) {
    return std::any_of(list.begin(), list.end(), [&target](const std::string& s) {
...
//#endif
  ...
  proc_t::running() {
    #if RAZER_MOD == 1
    auto appLaunchTimeout = 5s;
    if(isInList(_app.launch_type, {"BattleNet", "Uplay", "Epic", "Origin", "Mihoyo"}) == true
    || isInListContain(_app.cmd, {"Rockstar Games"}) == true){
      appLaunchTimeout = 100s;
    }
    #endif
    ...
    if (placebo) {
      ...
      #if RAZER_MOD == 1
      std::chrono::steady_clock::now() - _app_launch_time < appLaunchTimeout) {
      BOOST_LOG(info) << "App exited gracefully within "sv << appLaunchTimeout.count() <<" seconds of launch. Treating the app as a detached command."sv;
      #else
  ...
  proc_t::terminate() {
    ...
    _pipe.reset();
#if (defined SUNSHINE_TRAY && SUNSHINE_TRAY >= 1) || (RAZER_MOD == 1)
    ...
    if (proc::proc.get_last_run_app_name().length() > 0 && has_run) {
      ...
      #if defined RAZER_MOD && RAZER_MOD == 1
      rz_state::updateCurrentState("StreamStopped", proc::proc.get_last_run_app_name(), current_device_nickname);
      current_device_nickname = "";
      if(rz_state::IsVirtualDisplayCreated() == true){
        BOOST_LOG(info) << "StreamStopped notification";
        rz_state::updateEvent(Command_Protocol_VirtualDisplay_Plugout, "StreamStopped", proc::proc.get_last_run_app_name());
      }
    ...
    {
      //kill it any way
      if(isInList(_app.launch_type, {"Steam", "Xbox", "BattleNet", "Epic", "Origin", "Uplay", "Garena", "GOG", "Mihoyo"}) == true 
      || isInListContain(_app.cmd, {"Rockstar Games"}) == true)
        KillProcessByPID(_app.pid);
    }
#endif
...
#if (RAZER_GAME_LAUNCHER==1)
  bool
  proc_t::kill_running_game(){
    
    if(_app.pid.length() > 0)
      return KillProcessByPID(_app.pid);
    else{
      BOOST_LOG(warning) << "no game to kill";
      return false;
     }
  }

  void proc_t::appStart(std::string guid, std::string PID){
    if(_app.guid == guid)
    {
      _app.pid = PID;
      _app_actual_launch_time = std::chrono::steady_clock::now();
    }
  }

  void proc_t::appEnd(std::string guid){
    if(isInList(_app.launch_type, {"Uplay"} ) == true
    || isInListContain(_app.cmd, {"Rockstar Games"}) == true)
    {
      if(std::chrono::steady_clock::now() - _app_actual_launch_time < 5s)
      {
        BOOST_LOG(info) << _app.launch_type << ": game stops in 5 seconds of launch. Treating the app as a detached command";
        return;
      }
    }

    //once end event recieved, all session should be terminate
    if(running() != 0 && _app.guid == guid){
      BOOST_LOG(info) << "try to termiante application";
      terminate();
    }
    else{
      BOOST_LOG(info) << "No application running or exit application is not the running one";
    }
  }
#endif
  ...
  proc_t::get_app_image(int app_id) {
    ...
    auto app_image_path = iter == _apps.end() ? std::string() 
    #if (RAZER_GAME_LAUNCHER==1)
    : iter->custom_image_path;
    #else
    : iter->image_path;
    #endif

#if RAZER_MOD == 1
    if(iter->guid == DESKTOP_GUID)
      return app_image_path; //avoid Chinese/Japnese characters issue.
    else
#endif
  ...
  validate_app_image_path(std::string app_image_path) {
    ...
    #if (RAZER_GAME_LAUNCHER == 1)
    auto image_filename = std::filesystem::path(app_image_path).filename().string();
    boost::to_lower(image_filename);
    #endif
    boost::to_lower(image_extension);

    // return the default box image if extension is not "png"
    #if (RAZER_GAME_LAUNCHER == 1)
    if ( isInList(image_extension, {".webp", ".png", ".jpg", ".jpeg", ".bmp", ".gif", ".dib", ".tiff"}) == false
      && image_filename != "transcodedwallpaper" ) {      
    #else
  ...
  calculate_app_id(const std::string &app_name, std::string app_image_path, int index) {
    ...
    #if (RAZER_GAME_LAUNCHER == 1)
    to_hash.push_back(app_image_path); //actually guid here, it's unique
    #else
  ...
  KillProcessByPID(const std::string& pidStr) {
      ...
      if (hProcess == NULL) {
          BOOST_LOG(error) << "Failed to open process. Error: " << GetLastError() << std::endl;
          return false;
      }

      if (!TerminateProcess(hProcess, 0)) {
          BOOST_LOG(error) << "Failed to terminate process. Error: " << GetLastError() << std::endl;
  ...
  parse(const std::string &file_name) {
    ...
    try {
      ...
      for (auto &[_, app_node] : apps_node) {
        ...
        #if (RAZER_GAME_LAUNCHER == 1) 
        auto guid = app_node.get_optional<std::string>("guid"s);
        auto custom_image_path = app_node.get_optional<std::string>("custom_image_path"s);
        auto launch_type = app_node.get_optional<std::string>("launch_type"s);
        auto monitor_exe = app_node.get_optional<std::string>("monitor_exe"s);
        #endif
...
#if (RAZER_GAME_LAUNCHER == 1)
        if(guid)
           ctx.guid = std::move(*guid);

        if(custom_image_path)
          ctx.custom_image_path = std::move(*custom_image_path);
          
        if(launch_type)
          ctx.launch_type = std::move(*launch_type);

        if(monitor_exe)
          ctx.monitor_exe = std::move(*monitor_exe);

        auto possible_ids = calculate_app_id(name, ctx.guid, i++);
#else
